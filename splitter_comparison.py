...
st.set_page_config(page_title="Splitter Comparison",layout="wide")
st.title("Splitter Comparison")
st.divider()

target_url=st.text_input(label="Enter the web address to be processed:")
st.divider()
chunk_size=st.slider(label="Determine the chunk size",min_value=100,max_value=2000,value=1000,step=100,key="url_chunk_size")
st.divider()
chunk_overlap=st.slider(label="Determine the chunk overlap size",min_value=0,max_value=1000,value=0,step=100,key="url_chunk_overlap")
st.divider()
submit_btn=st.button(label="Start",key="url_button")
st.divider()
